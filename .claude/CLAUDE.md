...
Preserve the AIDL identifiers, the Binder protocol and every wire-level string; the Java packages of Porter's own code are free to change, and everything Porter owns is Kotlin. SDK release versions are independent of protocol versions in `manifest.gradle` and `ShizukuApiConstants`.
